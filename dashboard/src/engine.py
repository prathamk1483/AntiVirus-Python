...
class Engine:
    def __init__(self, typeH):
        if typeH.lower() == "sha256":
            with open("src/DataBase/HashDataBase/Sha256/virusHash.txt", "r") as i:
                self.hashList = i.readlines()
                i.close()

        if typeH.lower() == "md5":
            with open("src/DataBase/HashDataBase/Md5/md5HashOfVirus.txt", "r") as i:
                self.hashList = i.readlines()
                i.close()
    ...
    def md5_hash(self, filename):
        try:
            ...
            print(f"Error calculating MD5 for {filename}: {str(e)}")
            return "0"
    ...
    def virusScannerMd5(self, path, scan_callback=None):
        self.virusPath = []
        self.virusHashCyPy = []
        ioList = [self.hashToFullNum(h) for h in self.hashList]
        ioList.sort()
        ...
        for file_path in dir_list:
            try:
                ...
                file_hash = self.md5_hash(file_path)
                hash_num = self.hashToFullNum(file_hash)
                vIHash = self.binaryTreeSearch(ioList, hash_num)
